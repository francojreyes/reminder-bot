...
class OverviewEmbed(discord.Embed):
        ...
        def __init__(self, bot: discord.Bot):
            ...
            self.add_field(name="About", value=f"This bot is developed by @marsh#0943. Add me to report any bugs!")
...
class CogEmbed(discord.Embed):
    ...
    def __init__(self, cog: discord.Cog):
        ...
        for command in cog_commands(cog):
            self.add_field(name=f"`/{command.qualified_name}`", value=command.description, inline=False)
...
class CommandEmbed(discord.Embed):
    ...
    def __init__(self, command: discord.ApplicationCommand):
        ...
        for option in command.options:
            self.add_field(
                name = f'`<{option.name}>`' + (' (Optional)' if not option.required else ''),
